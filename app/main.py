...
from app.tasks.example_tasks import ask_groq, ask_groq_2, GroqTaskPayload
...
@app.get("/jobs/{job_id}", response_model=JobStatusResponse, status_code=status.HTTP_200_OK)
...
groq_router = create_task_router(
    task=ask_groq,
    payload_model=GroqTaskPayload,
    task_name="ask-groq"
)
app.include_router(groq_router, prefix="/api", tags=["Groq Tasks"])

groq_router_2 = create_task_router(
    task=ask_groq_2,
    payload_model=GroqTaskPayload,
    task_name="ask-groq-2"
)
app.include_router(groq_router_2, prefix="/api", tags=["Groq Tasks 2"])
